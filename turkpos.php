        ...
        class WC_Param_Gateway extends WC_Payment_Gateway
        {
            ...
            public function init_form_fields()
            {
                $this->form_fields = array(
                    ...
                    'installment_2_min' => array(
                        'title' => '2 Taksit Minimum Tutar',
                        'type' => 'text',
                        'description' => '2 taksit için minimum sipariş tutarı (TL). Boş bırakılırsa devre dışı bırakılır.',
                        'default' => ''
                    ),
                    'installment_3_min' => array(
                        'title' => '3 Taksit Minimum Tutar',
                        'type' => 'text',
                        'description' => '3 taksit için minimum sipariş tutarı (TL). Boş bırakılırsa devre dışı bırakılır.',
                        'default' => ''
                    ),
                    'installment_4_min' => array(
                        'title' => '4 Taksit Minimum Tutar',
                        'type' => 'text',
                        'description' => '4 taksit için minimum sipariş tutarı (TL). Boş bırakılırsa devre dışı bırakılır.',
                        'default' => ''
                    ),
                    'installment_5_min' => array(
                        'title' => '5 Taksit Minimum Tutar',
                        'type' => 'text',
                        'description' => '5 taksit için minimum sipariş tutarı (TL). Boş bırakılırsa devre dışı bırakılır.',
                        'default' => ''
                    ),
                    'installment_6_min' => array(
                        'title' => '6 Taksit Minimum Tutar',
                        'type' => 'text',
                        'description' => '6 taksit için minimum sipariş tutarı (TL). Boş bırakılırsa devre dışı bırakılır.',
                        'default' => ''
                    ),
                    'installment_7_min' => array(
                        'title' => '7 Taksit Minimum Tutar',
                        'type' => 'text',
                        'description' => '7 taksit için minimum sipariş tutarı (TL). Boş bırakılırsa devre dışı bırakılır.',
                        'default' => ''
                    ),
                    'installment_8_min' => array(
                        'title' => '8 Taksit Minimum Tutar',
                        'type' => 'text',
                        'description' => '8 taksit için minimum sipariş tutarı (TL). Boş bırakılırsa devre dışı bırakılır.',
                        'default' => ''
                    ),
                    'installment_9_min' => array(
                        'title' => '9 Taksit Minimum Tutar',
                        'type' => 'text',
                        'description' => '9 taksit için minimum sipariş tutarı (TL). Boş bırakılırsa devre dışı bırakılır.',
                        'default' => ''
                    ),
                    'installment_10_min' => array(
                        'title' => '10 Taksit Minimum Tutar',
                        'type' => 'text',
                        'description' => '10 taksit için minimum sipariş tutarı (TL). Boş bırakılırsa devre dışı bırakılır.',
                        'default' => ''
                    ),
                    'installment_11_min' => array(
                        'title' => '11 Taksit Minimum Tutar',
                        'type' => 'text',
                        'description' => '11 taksit için minimum sipariş tutarı (TL). Boş bırakılırsa devre dışı bırakılır.',
                        'default' => ''
                    ),
                    'installment_12_min' => array(
                        'title' => '12 Taksit Minimum Tutar',
                        'type' => 'text',
                        'description' => '12 taksit için minimum sipariş tutarı (TL). Boş bırakılırsa devre dışı bırakılır.',
                        'default' => ''
                    ),
                    'pos_rates' => array(
            ...
            public function validate_fields()
            {
                ...
                // Validate installment eligibility based on cart total
                if (!empty($_POST['cc_installment'])) {
                    global $woocommerce;
                    $cart_total = (float)$woocommerce->cart->total;
                    $installment_data = explode('|', $_POST['cc_installment']);
                    $installment_count = (int)$installment_data[0];

                    // Check if installment count is eligible based on minimum amount
                    if ($installment_count > 1) {
                        $min_amount_key = "installment_{$installment_count}_min";
                        $min_amount = $this->get_option($min_amount_key);

                        if (empty($min_amount)) {
                            wc_add_notice('Seçilen taksit seçeneği geçerli değil.', 'error');
                            return false;
                        }

                        if ($cart_total < (float)$min_amount) {
                            wc_add_notice("Bu taksit seçeneği için minimum sipariş tutarı {$min_amount} TL olmalıdır.", 'error');
                            return false;
                        }
                    }
                }
...
<soap:Envelope xmlns:xsi="http://www.w3.org/2001/XMLSchema-instance" xmlns:xsd="http://www.w3.org/2001/XMLSchema" xmlns:soap="http://schemas.xmlsoap.org/soap/envelope/">
  <soap:Body>
    ...
    function checkout_billing_email_js_ajax()
    {
        ...
        $has_eligible_installments = false;

        // Check if any installment option is eligible based on cart total
        for ($i = 2; $i <= 12; $i++) {
            $min_amount = $paramGateway->settings["installment_{$i}_min"];
            if (!empty($min_amount) && $subtotal >= (float)$min_amount) {
                $has_eligible_installments = true;
                break;
            }
        }

        if (is_checkout() && !is_wc_endpoint_url() && $paramGateway->settings['installment'] !== 'no' && $has_eligible_installments)  :
            ?>
            ...
            <script type="text/javascript">
                jQuery(function ($) {
                    ...
                    // Listen for cart updates to refresh installment options
                    $(document.body).on('updated_checkout', function() {
                        var ccNumber = $('#ccpp_creditcard_cc_number').val();
                        if (ccNumber && ccNumber.replace(/\s+/g, '').length >= 6) {
                            // Trigger installment refresh when cart is updated
                            $('#ccpp_creditcard_cc_number').trigger('keyup');
                        }
                    });
                });
            </script>
                ...
                if (isset($bin_response["posId"])) {
                    ...
                    foreach ($response as $key => $resp) {
                        if ($resp[0]["SanalPOS_ID"] == $posId) {
                            //Eğer dönen "cardType" Debit Card'a eşitse sadece ilk taksidi dönder. Debit Card değilse dinamik taksit kontrolü yap.
                            $maxInstallments = ($bin_response['cardType'] === "Debit Kart" || $bin_response['cardType'] === "Debit Card") ? 1 : 12;
                            $subtotal = (float)$woocommerce->cart->total;
                            ...
                            for ($i = 1; $i <= $maxInstallments; $i++) {
                                ...
                                // Check if installment is eligible based on minimum amount requirements
                                if ($i > 1) {
                                    $min_amount = $paramGateway->settings["installment_{$i}_min"];
                                    if (empty($min_amount) || $subtotal < (float)$min_amount) {
                                        continue; // Skip this installment if not eligible
                                    }
                                }
